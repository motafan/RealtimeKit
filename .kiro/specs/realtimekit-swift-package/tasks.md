...
- [x] 2. 实现核心数据模型和验证逻辑（依赖任务 3.3, 3.4）
  - [x] 2.1 创建用户角色和权限系统
...
- [x] 3. 实现存储管理系统
  ...
  - [x] 3.1 迁移音频设置到自动持久化机制（依赖任务 3.3, 3.4）
    - 重构现有的 AudioSettingsStorage 类，使用新的 @RealtimeStorage 属性包装器
    - 移除手动 JSON 序列化逻辑，依赖自动持久化机制
    - 更新错误处理逻辑，集成新的存储后端错误处理
    - 重写单元测试，验证 @RealtimeStorage 的音频设置持久化功能
    - 确保向后兼容性和数据迁移支持
    - _需求: 5.4, 5.5, 18.1, 18.2, 18.3_

  - [x] 3.2 迁移用户会话到自动持久化机制（依赖任务 3.3, 3.4）
    - 重构现有的 UserSessionStorage 类，使用新的 @RealtimeStorage 属性包装器
    - 对于敏感会话数据，使用 @SecureRealtimeStorage 进行安全存储
    - 移除手动存储和恢复逻辑，依赖自动持久化机制
    - 重写单元测试，验证会话数据的自动持久化和恢复功能
    - 确保数据完整性和安全性验证
    - _需求: 4.4, 4.5, 18.1, 18.2, 18.5_
...
- [x] 12.4 修复国际化实现使用标准 Localizable.strings
  - 将字典方式的本地化字符串迁移到标准的 Localizable.strings 文件
  - 重构 LocalizationManager 使用 NSLocalizedString 而不是字典查找
  - 创建多语言的 .lproj 目录和 Localizable.strings 文件
  - 更新 Package.swift 包含资源文件配置
  - 确保符合 iOS 开发最佳实践和标准
  - _需求: 17.1, 17.2, 17.4, iOS 标准实践_
...
- [ ] 17. 完善测试覆盖 (使用 Swift Testing 框架)
  - [ ] 17.1 迁移到 Swift Testing 框架
    - 将现有 XCTest 测试迁移到 Swift Testing 框架
    - 使用 `@Test` 宏替代传统的 XCTest 方法
    - 实现 `import Testing` 替代 `import XCTest`
    - 更新 Package.swift 配置以支持 Swift Testing
    - _需求: 16.1, 测试要求 1_

  - [ ] 17.2 实现参数化和条件测试
    - 使用 `@Test(arguments:)` 实现数据驱动的参数化测试
    - 实现 `@Test(.enabled(if:))` 条件测试执行
    - 为音频设置、用户角色、音量检测等核心功能编写参数化测试
    - 添加本地化功能的多语言参数化测试
    - 添加环境变量控制的条件测试（如服务商特定测试）
    - _需求: 16.2, 测试要求 1, 17.1_

  - [ ] 17.3 扩展单元测试套件
    - 为所有管理器和存储类编写完整的 Swift Testing 单元测试
    - 使用 `#expect()` 断言替代 XCTest 断言
    - 实现异步测试支持 `async throws` 测试方法
    - 添加本地化管理器和错误处理的测试覆盖
    - 为自动状态持久化机制编写完整的单元测试套件
    - 测试 @RealtimeStorage 和 @SecureRealtimeStorage 属性包装器功能
    - 添加存储后端和数据迁移的测试覆盖
    - 添加错误处理和异常情况的测试覆盖
    - 确保测试覆盖率达到 80% 以上
    - _需求: 16.1, 16.2, 16.3, 测试要求 1, 17.1, 18.11_

  - [ ] 17.4 实现集成测试和性能测试
    - 创建多服务商兼容性集成测试（使用 Swift Testing）
    - 实现网络异常和并发操作测试
    - 添加本地化功能的集成测试和性能测试
    - 为持久化机制编写集成测试，验证跨应用重启的状态恢复
    - 添加存储性能基准测试和批量操作测试
    - 测试数据迁移的完整性和性能
    - 添加性能基准测试和内存泄漏检测
    - 编写端到端的功能测试场景
    - 使用 Swift Testing 的测试套件组织功能
    - _需求: 16.4, 14.1, 测试要求 1, 17.1, 18.8, 18.11_
